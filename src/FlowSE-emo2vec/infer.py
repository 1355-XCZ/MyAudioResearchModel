...
def run(args):
    ...
    print("decode data: ", conf["datareader"]["vevo_mel_json"])
    ...
    model_cls = DiT
    nnet = CFM(transformer=model_cls(**conf['nnet_conf']['arch'], mel_dim=conf['nnet_conf']['mel_spec']['n_mel_channels']),
        mel_spec_kwargs=conf['nnet_conf']['mel_spec'], emotion_processor=emotion_processor).eval().to(device)
    ...
    data_reader = EmotionDataReader(**conf["datareader"])
    ...
    with torch.no_grad():
        ...
        for egs in tqdm(data_reader):
            ...
            vevo_mel = egs["vevo_mel"].contiguous().to(device)
            emotion_features = {
                k: v.to(device) for k, v in egs["emotion_features"].items()
            }
            
            if conf['test']['cond_type'] == 'emotion':
                # Use emotion conditioning
                output, _ = nnet.sample(cond=vevo_mel, emotion_features=emotion_features)
            elif conf['test']['cond_type'] == 'no_emotion':
                # No emotion conditioning
                output, _ = nnet.sample(cond=vevo_mel, emotion_features=None, drop_emotion=True)
