...
from model import DiT,CFM, EmotionProcessor
from model.model_utils import get_tokenizer


import os
from loader.dataloader import make_auto_loader
from loader.emotion_dataloader import make_emotion_loader
...
def make_dataloader(opt):
    ...
    train_sampler, train_loader = make_emotion_loader(
        **opt["datasets"]["train"],
        **opt["datasets"]["dataloader_setting"],
    )
    val_sampler, val_loader = make_emotion_loader(
        **opt["datasets"]["val"],
        **opt["datasets"]["dataloader_setting"],
    )
...
def train_one_epcoh(
    ...
    for i, egs in enumerate(train_loader):
        ...
        vevo_mel = egs["vevo_mel"].transpose(-1,-2)
        source_mel = egs["source_mel"].transpose(-1,-2)
        emotion_features = egs["emotion_features"]
    
        loss, _, _ = nnet(inp=vevo_mel, clean=source_mel, emotion_features=emotion_features)
    
        torch.distributed.barrier()
        reduced_loss = reduce_mean(loss, world_size)
        losses.update(reduced_loss.item(), vevo_mel.size(0))
...
def validate_one_epcoh(
    ...
    with torch.no_grad():
        ...
        for i, egs in enumerate(val_loader):
            ...
            loss, _, _ = nnet.module.forward(inp=vevo_mel, emotion_features=emotion_features, clean=source_mel)
...
def main_worker(local_rank, args):
    ...
    emotion_processor = EmotionProcessor(device=device)
    
    nnet = CFM(
        transformer=model_cls(**conf['model']['arch'], mel_dim=conf['model']['mel_spec']['n_mel_channels']),
        audio_drop_prob=conf['model']['audio_drop_prob'],cond_drop_prob=conf['model']['cond_drop_prob'],
        mel_spec_kwargs=conf['model']['mel_spec'],emotion_processor=emotion_processor
    )
