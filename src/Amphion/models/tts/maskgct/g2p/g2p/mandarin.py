...
resource_path = str(current_file_dir)
...
word_pinyin_dict = {}
chinese_lexicon_path = os.path.join(
    resource_path, "sources", "chinese_lexicon.txt"
)
with open(
    chinese_lexicon_path, "r", encoding="utf-8"
) as fread:
...
pinyin_2_bpmf_path = os.path.join(
    resource_path, "sources", "pinyin_2_bpmf.txt"
)
with open(
    pinyin_2_bpmf_path, "r", encoding="utf-8"
) as fread:
    txt_list = fread.readlines()
    for txt in txt_list:
...
bpmf_2_pinyin_path = os.path.join(
    resource_path, "sources", "bpmf_2_pinyin.txt"
)
with open(
    bpmf_2_pinyin_path, "r", encoding="utf-8"
) as fread:
    txt_list = fread.readlines()
    for txt in txt_list:
        v, k = txt.strip().split("\t")
