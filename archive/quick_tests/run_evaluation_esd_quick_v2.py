...
def main():
    logger.info("="*80)
    logger.info("ESD快速评估V2（20样本/情感，码率≤50 BPF）")
    logger.info("每个样本单独搜索λ + 先验加速")
    logger.info("="*80)
    
    config = get_default_config()
    
    # 重点测试5-50 BPF + 无量化baseline
    quick_rates = [5, 10, 15, 20, 25, 30, 40, 50, float('inf')]
    quick_layers = [12, 24, 36, 48, 60]  # 12组×1~5层/组
    
    logger.info(f"码率点: {quick_rates}")
    logger.info(f"层数点: {quick_layers}")
    ...
    # 独立输出目录
    output_dir = Path('evaluation_quick_v2_results')
